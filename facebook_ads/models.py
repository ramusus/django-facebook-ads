...
ADCREATIVE_TYPE_CHOICES = (
    ...
    (25, _('Sponsored stories about likes for a specific page post, comments for a specific page post, and RSVPs for a specific event')),
...
class AdUser(FacebookGraphModel):
    ...
    permissions = models.CommaSeparatedIntegerField(max_length=20, choices=ADUSER_PERMISSIONS_CHOICES, help_text='An array of the permissions that user has.')
...
class AdAccount(FacebookGraphModel):
    ...
    account_status = models.SmallIntegerField(choices=ACCOUNT_STATUS_CHOICES, null=True, blank=True, help_text='Status of the account (1 for active, 2 for disabled, or 3 for unsettled)')
    daily_spend_limit = models.IntegerField(null=True, blank=True, help_text='The account\'s limit for daily spend, based on the corresponding value in the account settings')
    currency = models.CharField(max_length=100, blank=True, help_text='The currency used for the account, based on the corresponding value in the account settings')
...
#    account_groups = models._Field(help_text='Container for the ID, name, and status of the account\'s account groups')    ads_management    array
    ...
    business_street = models.CharField(max_length=100, blank=True, help_text='First line of the business street address for the account')
    business_street2 = models.CharField(max_length=100, blank=True, help_text='Second line of the business street address for the account')
    ...
    business_country_code = models.CharField(max_length=100, blank=True, help_text='Country code for the business address')
    vat_status = models.IntegerField(null=True, blank=True, help_text='Vat status code for the account. Here are the values for VAT Status: 0: Unknown, 1: VAT not required- US/CA, 2: VAT information required, 3: VAT information submitted, 4: Offline VAT validation failed, 5: Account is a personal account')
#    agency_client_declaration = models._Field(help_text='Whether this account is an agency advertising on behalf of the client') ads_management    array
    ...
    def fetch_groups(self):
        ...
        '''
        response = graph('act_%s/adgroups' % self.account_id)
    ...
    def fetch_stats(self):
        ...
        '''
        response = graph('act_%s/stats' % self.account_id)
    ...
    def fetch_stats_campaigns(self):
        ...
        '''
        response = graph('act_%s/adcampaignstats' % self.account_id)
...
class AdCampaign(FacebookGraphModel):
    ...
    name = models.CharField(max_length=100, help_text='Campaign name of 100 characters or less; required to create a new campaign')
    start_time = models.DateTimeField(null=True, blank=True, help_text='A unix timestamp; optional when creating and updating campaigns')
    end_time = models.DateTimeField(null=True, blank=True, help_text='A unix timestamp; optional when creating and updating campaigns')
    daily_budget = models.IntegerField(null=True, blank=True, help_text='Defined in cents, minimum 100; daily or lifetime budget must be set when creating a a campaign; can be updated')
    campaign_status = models.SmallIntegerField(choices=CAMPAIGN_STATUS_CHOICES, null=True, blank=True, help_text='A campaign status is active (status 1), paused (status 2), or deleted (status 3)')
    lifetime_budget = models.IntegerField(null=True, blank=True, help_text='Defined in cents, minimum 100; can be updated; either a lifetime_budget or a daily_budget can be set (not both), and an updated budget begins spending from zero (if there\'s a switch in budget types, the entire campaign-spend, to date, is set to zero); if money already was spent on the campaign today, that money is not counted towards the new budget; an advertiser may be charged up to the sum of the old spend, plus any new spend that occurs in the campaign; for more information, see the Facebook Help')
    ...
    def parse(self, response):
        ...
        try:
            ...
            raise Exception('Impossible to save campaign "%s" (%d) for unexisted account %s' % (self.name, self.campaign_id, response['account_id']))
    ...
    def fetch_stats(self):
        ...
        '''
        response = graph('%s/stats' % self.campaign_id)
        instance = AdStatistic.remote.get_or_create_from_resource(response)
        return instance
...
class AdCreative(FacebookGraphModel):
    ...
    type = models.IntegerField(choices=ADCREATIVE_TYPE_CHOICES, null=True, help_text='The number of the ad type, which identifies the type of sponsored story or ad')
    object_id = models.BigIntegerField(null=True, blank=True, help_text='The Facebook object ID that is relevant to the ad and ad type')
    body = models.CharField(max_length=135, help_text='The body of the ad, not applicable to sponsored stories')
    image_hash = models.CharField(max_length=32, help_text='Image ID for an image you can use in creatives and thus in ads')
    image_url = models.URLField(max_length=100, help_text='A URL for the image for this creative')
    count_current_adgroups = models.CharField(max_length=100, help_text='Indicates the number of ad groups in which the creative is used')
    title = models.CharField(max_length=25, help_text='Title for a default ad')
    run_status = models.SmallIntegerField(choices=ADCREATIVE_RUN_STATUS_CHOICES, null=True, help_text='Indicates whether the creative is available or deleted')
    link_url = models.URLField(max_length=1024, help_text='A URL for the ad')
    preview_url = models.URLField(max_length=100, help_text='The URL to preview the ad, only for the current session user')
    related_fan_page = models.CharField(max_length=100, help_text='Provides social context to a type 1 ad')
    auto_update = models.BooleanField(help_text='Boolean true to constantly promote the latest page post and ignore story_id parameter. Boolean false to promote a specific page post by story_id. Required for type 27 ads only.')
    story_id = models.BigIntegerField(null=True, blank=True, help_text='The fbid of a page post to use in a type 25 or type 27 ad. This ID can be retrieved by using the graph API to query the posts of the page.')
#    query_templates = models._Field(help_text='An array of numbers corresponding to the type of sponsored story for a type 25 ad (query-based sponsored stories). Current valid values are 6 (likes for a specific page post), 7 (comments for a specific page post), and 8 (RSVPs for a specific page event).') ads_management    array of int
    ...
    def clean(self):
        ...
        for word in words:
            if len(word) > 20:
                raise ValidationError('The maximum allowable length of an individual word in an ad’s body : 20 characters')
        words = self.title.split()
        for word in words:
            if len(word) > 20:
                raise ValidationError('The maximum allowable length of an individual word in an ad’s title : 20 characters')
...
class Targeting(FacebookGraphModel):
    ...
    # Location
    countries = fields.CommaSeparatedCharField(max_length=100, blank=True, help_text='Values for country targeting. The countries parameter is the only required targeting parameter. It must be an array of country codes. See Retrieving Autocomplete Data. Limit: 25 countries.')
    cities = fields.JSONField(null=True, blank=True, help_text='Values for city targeting. For the values you can use, see Retrieving Autocomplete Data. For cities in the US or Canada, specify the two letter state or province code, e.g. "name":"New York,NY" (or for international cities, the country name, e.g. "name":"Paris,France"). Alternatively, specify "id":"2421215","name":"Palo Alto, CA". Limit: 200 cities.')
    zips = models.CharField(max_length=100, help_text='String array of 5-digit ZIP codes (postal codes) specified by the United States Postal Service. For example: \'94304\'. Currently, the zips parameter does not support international postal codes, autocomplete, the radius parameter, or a countries parameter specification outside of \'US\'. See Zip Code Targeting. Limit: 2500 zip codes.')
    regions = models.CharField(max_length=100, help_text='State, province, or region. To get the available values, see Retrieving Autocomplete Data. Limit: 200 regions.')
    radius = models.PositiveSmallIntegerField(null=True, blank=True, help_text='Must be used in conjunction with cities. Specifies a radius around cities.')
    locales = models.CharField(max_length=100, help_text='Locales. To get the available values, see Retrieving Autocomplete Data. Alternatively, if you use the Autocomplete file (referenced above), the values are specified as indices; the indices are in a sub-array called \'locales\'. Uses of this parameter include for targeting users with a different language than the common language for the specified location parameter; you specify an ID for the language, e.g. 5 for German. Limit: 50 locales.')

    # Likes and interests
    keywords = fields.CommaSeparatedCharField(max_length=4000, help_text='Array of keyword strings for precise interest targeting. The keywords parameter applies only to precise interest targeting. See topic targeting. Keywords (either topic keywords or non-topic keywords) are matched to data from users profiles. For example, if an advertiser sells cameras, targeting parameters could specify users who are interested in photography. You can POST targeting data to /act_{account-id}/reachestimate to determine if proposed keywords match any profiles. Limit: 200 keywords.')
    user_adclusters = fields.JSONField(null=True, blank=True, help_text='Array of ID-name pairs for broad-category targeting (BCT). The user_adclusters parameter applies only for BCT, which cannot be used with precise interest targeting. To include BCT in an ad, use act_{account-id}/broadtargetingcategories. For an example of how to use this parameter, see broad category targeting, above. Limit: 50 ID-name pairs.')
    interested_in = models.CharField(max_length=100, help_text='Indicates targeting based on the "interested in" field of the user profile. You can specify an integer of 1 to indicate MALE (for example, if you specify "interested_in":[1]). An integer of 2 indicates FEMALE. The default of ALL is used if you specify Null. The default of ALL is also used if you do not specify the field. Do not specify zero. Please note "interested in" targeting is not available in France due to local laws.')

    # Demographics and events
    genders = fields.CommaSeparatedCharField(choices=TARGETING_GENDER_CHOICES, max_length=5, null=True, blank=True, help_text='Indicates gender-based targeting. You can specify an integer of 1 to indicate MALE (for example, if you specify "genders":[1]). An integer of 2 indicates FEMALE. The default of ALL is used if you specify Null. The default of ALL is also used if you do not specify the field. Do not specify zero.')
    age_min = fields.PositiveSmallIntegerRangeField(min_value=13, max_value=65, null=True, blank=True, help_text='Minimum age. If used, must be 13 or higher.')
    age_max = fields.PositiveSmallIntegerRangeField(min_value=13, max_value=65, null=True, blank=True, help_text='Maximum age. If used, must be 65 or lower.')
    broad_age = models.SmallIntegerField(choices=TARGETING_BROAD_AGE_CHOICES, default=0, help_text='Whether or not to enable "broad-age-match" targeting. The default is 0 (disabled). Set to 1 to enable broad-age-match targeting.')
    relationship_statuses = models.CommaSeparatedIntegerField(null=True, blank=True, max_length=100, help_text='Array of integers for targeting based on relationship status. Use 1 for SINGLE, 2 for IN_RELATIONSHIP, 3 for MARRIED, and 4 for ENGAGED. The default of ALL is used if you specify Null. The default of ALL is also used if you do not specify the field. Do not specify zero.')
    user_event = models.CommaSeparatedIntegerField(max_length=100, help_text='Array of integers corresponding to user-relevant events. Currently only supports birthday. To use birthday, pass a one-member array with a value of 1, e.g. "user_event":[1].')

    # Facebook connections
    connections = models.CommaSeparatedIntegerField(max_length=100, help_text='An array of Facebook IDs. Connections allows you to target your ads to users who have become a fan of your Page, a member of your Group, RSVP’d to your Event or authorized your Application. The session user has to be an admin of the page, group or event, or developer of the application that is being targeted against. Limit: 50 connections.')
    excluded_connections = fields.JSONField(null=True, blank=True, help_text='An array of Facebook IDs. Excluded connections allows you to target your ads to users who have not become fans of your Page, members of your Group, RSVP\'d to your Event or authorized your Application. The session user has to be an admin of the page, group or event, or developer of the application that is being targeted against.')
    friends_of_connections = fields.JSONField(null=True, blank=True, help_text='An array of Facebook IDs. "Friends of connections" allows you to target friends of your connections. Connections are fans of your Page, users who have RSVP’d Yes or Maybe to your Event, members of your Group, and users who have interacted with your app. The session user has to be an admin of the page, group or event, or a developer of the application that is being targeted against.')

    # Education and workplace
    college_networks = models.CharField(max_length=100, help_text='Colleges, for college graduates. Limit: 200 college networks.')
    work_networks = models.CharField(max_length=100, help_text='Company, organization, or other workplace. Limit: 200 work networks.')
    education_statuses = fields.CommaSeparatedCharField(max_length=100, help_text='Array of integers for targeting based on education level. Use 1 for HIGH_SCHOOL, 2 for UNDERGRAD, and 3 for ALUM. For one ad group, you can specify only one value. The education_statuses parameter is required if you specify the college_majors parameter.')
    college_years = models.CommaSeparatedIntegerField(max_length=100, help_text='Array of integers for graduation year from college.')
    college_majors = fields.CommaSeparatedCharField(max_length=100, help_text='Array of strings for specific college majors. Requires that you also specify a value for the education_statuses parameter. Limit: 200 college majors.')
...
class AdGroup(FacebookGraphModel):
    ...
    campaign = models.ForeignKey(AdCampaign, related_name='adgroups', help_text='ID of the campaign, required for creating ad groups')
    # in API docs max_length=25, but there is bigger values in the real
    name = models.CharField(max_length=100, help_text='The name of the ad group, not longer than 25 characters, required for creating ad groups and can be updated')
    adgroup_status = models.IntegerField(choices=ADGROUP_STATUS_CHOICES, null=True, help_text='1 indicates ACTIVE, 3 indicates DELETED, 4 indicates PENDING_REVIEW, 5 indicates DISAPPROVED, 8 indicates CAMPAIGN_PAUSED (resuming the Campaign resumes the ad group) and 9 indicates ADGROUP_PAUSED, and if the status is ADGROUP_PAUSED the ad group stays in that state regardless of the campaign being PAUSED or ACTIVE because you would have to resume that ad group specifically')
    bid_type = models.IntegerField(choices=BID_TYPE_CHOICES, null=True, help_text='Use 1 for CPC and 2 for CPM; required when creating ad groups;')
    max_bid = models.IntegerField(null=True, help_text='The highest bid, defined in cents; required when creating ad groups; can be updated')
    targeting = models.OneToOneField(Targeting, null=True, related_name='adgroup', help_text='The ad\'s targeting elements')
    # in API docs `creative` atribute, but `creative_ids` in ther real
    creative = models.ForeignKey(AdCreative, null=True, related_name='adgroups', help_text='The ad\'s creative elements')
...
#    disapprove_reason_descriptions = models._Field(help_text='Reason ad was not approved') ads_management    array
    ...
    def parse(self, response):
        ...
        try:
            ...
            raise Exception('Impossible to save adgroup "%s" (%d) for unexisted campaign %s' % (self.name, self.adgroup_id, response['campaign_id']))

        try:
            self.account = AdAccount.objects.get(account_id=response['account_id'])
        except AdAccount.DoesNotExist:
            raise Exception('Impossible to save adgroup "%s" (%d) for unexisted account %s' % (self.name, self.adgroup_id, response['account_id']))
    ...
    def save(self, *args, **kwargs):
        ...
        '''
        try:
            # because of strange error while saving adgroup.save() - IntegrityError: null value in column "targeting_id" violates not-null constraint
        ...
        try:
            # because of strange error while saving adgroup.save() - IntegrityError: null value in column "creative_id" violates not-null constraint
...
#            print row
            ...
            if 'Campaign ID' in row:
                campaign, created = AdCampaign.objects.get_or_create(campaign_id=row['Campaign ID'], defaults={'account': account, 'name': row['Campaign']})
                if not created:
                    campaign.name = row['Campaign']
                    campaign.save()
            if 'Ad ID' in row:
                adgroup, created = AdGroup.objects.get_or_create(adgroup_id=row['Ad ID'], defaults={'campaign': campaign, 'account': account, 'name': row['Ad Name']})
                if not created:
            ...
            if 'Date Range' in row:
                ...
                raise CSVFormatError("Are you sure language of export file is English? Could not find start_time/end_time fields in csv row: %s" % row)
...
class AdStatistic(FacebookGraphModel):
    ...
    account = models.ForeignKey(AdAccount, null=True, related_name='adstatistics', help_text='The ID of the object the statistics refer to, this may also be adcampaign_id or adgroup_id')
    campaign = models.ForeignKey(AdCampaign, null=True, related_name='adstatistics', help_text='The ID of the object the statistics refer to, this may also be adcampaign_id or adgroup_id')
    adgroup = models.ForeignKey(AdGroup, null=True, related_name='adstatistics', help_text='The ID of the object the statistics refer to, this may also be adcampaign_id or adgroup_id')

    start_time = models.DateTimeField(null=True, blank=True, help_text='A unix timestamp of the stats of the statistics period')
    end_time = models.DateTimeField(null=True, blank=True, help_text='A unix timestamp of the stats of the statistics period')

    impressions = models.IntegerField(null=True, help_text='The total number of times your adverts have been shown on the site.')
    clicks = models.IntegerField(null=True, help_text='The number of clicks the Sponsored Story or advert has received. If you\'re promoting a Page, event or application, clicks will also include Page likes, event RSVPs or installation of applications directly from the Sponsored Story or advert.')
    ...
    social_spent = models.IntegerField(null=True, help_text='The amount spent during the selected time period on ads with social context.')
    actions = models.IntegerField(null=True, help_text='The number of action performed on this ad')
    unique_impressions = models.IntegerField(null=True, help_text='The number of individuals this ad was shown to on the site')
    unique_clicks = models.IntegerField(null=True, help_text='The number of individuals who clicked this ad.')
    social_unique_impressions = models.IntegerField(null=True, help_text='The number of individuals this ad was shown to with social context')
    social_unique_clicks = models.IntegerField(null=True, help_text='The number of individuals who clicked this ad while it had social context')
    connections = models.IntegerField(null=True, help_text='The number of events (likes, RSVPs, app installs, joins to a group) attributed to an ad or campaign within 24 hours of the user viewing or clicking an ad. The Connections stat is incremented by one of the following: For a given 24-hour period, if no click has occurred but an impression has occurred, the ad with the latest impression has the Connection stat incremented by one. For a given 24-hour period, if a click has occurred, the ad with the latest click has the Connection stat incremented by one.')

    # only in csv reports
    social_reach = models.IntegerField(null=True, help_text='How many people saw your ad or Sponsored Story with the names of their friends who had already liked your Page, RSVPed to your event, or used your app.')
    ...
    page_likes = models.IntegerField(null=True, help_text='The number of people who like your Page within 24 hours of viewing an ad or sponsored story, or within 28 days after clicking on it.')
    ...
    ctr = models.FloatField(help_text='Click-through rate, or the number of clicks your Sponsored Story or ad received divided by the number of times it was shown on the site.')
    social = models.FloatField(help_text='The percentage of impressions where your ad was shown with the names of viewers\' friends who liked your Page, RSVPed to your event, or used your app.')
    social_ctr = models.FloatField(help_text='The number of social clicks received divided by the number of social impressions.')
    cpc = models.FloatField(help_text='The average cost per click for these ads, calculated as the amount spent divided by the number of clicks received.')
    cpm = models.FloatField(help_text='The average cost per thousand impressions on these ads, calculated as the amount spent divided by the the number of impressions received.')
    unique_ctr = models.FloatField(help_text='The unique click through rate. The unique CTR is calculated as the number of unique clicks the ad(s) received divided by the number of unique ad impressions.')
    ...
    def parse(self, response):
        ...
        try:
            ...
            raise Exception('Impossible to save statistic "%s" for unexisted campaign %s' % (self.statistic_id, response['campaign_id']))
        except KeyError:
            pass
        try:
            self.adgroup = AdGroup.objects.get(adgroup_id=response['adgroup_id'])
        except AdGroup.DoesNotExist:
            raise Exception('Impossible to save statistic "%s" for unexisted adgroup %s' % (self.statistic_id, response['adgroup_id']))
        except KeyError:
            pass
        try:
            self.account = AdAccount.objects.get(account_id=response['account_id'])
        except AdAccount.DoesNotExist:
            raise Exception('Impossible to save statistic "%s" for unexisted account %s' % (self.statistic_id, response['account_id']))
        except KeyError:
            pass
    ...
    def set_auto_values(self):
        ...
        if not self.unique_ctr:
            self.unique_ctr = float('%.3f' % (100*float(self.unique_clicks)/self.unique_impressions) if self.unique_impressions else 0)
        if not self.social_ctr:
            self.social_ctr = float('%.3f' % (100*float(self.social_clicks)/self.social_impressions) if self.social_impressions else 0)
        if not self.social:
            self.social = float('%.2f' % (100*float(self.social_impressions)/self.impressions) if self.impressions else 0)
        if not self.cpc:
...
class AdImage(FacebookGraphModel):
    ...
    url = models.URLField(max_length="100", unique=True, help_text='A temporary URL which the image can be retrieved at.')
